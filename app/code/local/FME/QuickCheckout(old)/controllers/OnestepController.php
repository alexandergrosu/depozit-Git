...
class FME_QuickCheckout_OnestepController extends Mage_Checkout_OnepageController
{
		 ...
		 $billingData       = $this->_filterPostData($billingPostData);
            ...
            if (isset($resultBilling['error'])) {
                if(is_array($resultBilling['message'])){
                   $html="";
                   foreach($resultBilling['message'] as $msg){
                    $html.=$msg . " and ";
                   }
                   $result['error_messages'] = 'Billing Error: ' . $html;
                }else{
