        ...
        if ($this->getCollection()) {
            ...
            if ($column->getFilterConditionCallback()) {
               ...
               $cond = $column->getFilter()->getCondition();
				...
				if ($field == "billing_name" && isset($cond)) {
					$filterOrig = $cond['like'];
					$namesArray = explode(' ', $filterOrig);
					$like = array();
        				foreach ($namesArray as $item)
        				{
        					$itemreplaced = str_replace(array("%","'"),"", $item);
            				$like[] = "'%".$itemreplaced."%'";
        				}
						$likeimp = implode(" OR ",$like);
						print_r($likeimp."<br/>");
						$newZendDbExpr = new Zend_Db_Expr($likeimp);
						$modifCond = array('like'=>$newZendDbExpr);
							print_r($modifCond);
						$this->getCollection()->addFieldToFilter($field , $modifCond);
				} 
				
				else if($field == "name" && isset($cond)) {
					$filterOrig = $cond['like'];
