...
class Mage_Banners_Model_Mysql4_Banners extends Mage_Core_Model_Mysql4_Abstract
{
	 ...
	 protected function _afterLoad(Mage_Core_Model_Abstract $object)
    {
        ...
        $select = $this->_getReadAdapter()->select()
            ->from($this->getTable('banners/banners_store'))
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        ...
        $this->_getWriteAdapter()->delete($this->getTable('banners/banners_store'), $condition);
    
        foreach ((array)$object->getData('stores') as $store) {
            $storeArray = array();
            $storeArray['banners_id'] = $object->getId();
            $storeArray['store_id'] = $store;
            $this->_getWriteAdapter()->insert($this->getTable('banners/banners_store'), $storeArray);
        }
